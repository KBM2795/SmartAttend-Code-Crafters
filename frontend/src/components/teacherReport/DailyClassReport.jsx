...
const DailyClassReport = ({ data }) => {
  if (!data?.summary) return null;

  const timeChartData = data.summary.timeSlots.map(slot => ({
    time: format(new Date(slot.startTime), 'hh:mm a'),
    Present: slot.present,
    Absent: slot.absent,
    Rate: ((slot.present / (slot.present + slot.absent)) * 100).toFixed(1)
  }));
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-gray-800 p-4 rounded-lg">
          <h4 className="text-green-400">Present Today</h4>
          <p className="text-2xl font-bold text-green-500">{data.summary.present}</p>
        </div>
        <div className="bg-gray-800 p-4 rounded-lg">
          <h4 className="text-red-400">Absent Today</h4>
      ...
      <div className="bg-gray-800 p-4 rounded-lg">
        <h3 className="text-lg font-semibold mb-4">Time-wise Attendance</h3>
        <div className="h-64">
          ...
          <tbody>
            {data.records.map(record => 
              record.students.map(student => (
                <tr key={student.student._id} className="border-b border-gray-700">
                  ...
                  <td className={`p-2 ${
                    ...
                    {student.subjects[0].status.toUpperCase()}
                  </td>
                  <td className="p-2">
